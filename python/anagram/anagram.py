...
def is_anagram(word_primes_product, anagram):
    return word_primes_product == primes_product(anagram)
    
def detect_anagrams(word, anagrams):
    gen_first_n_primes(26)
    word_primes_product = primes_product(word)
    i = 0
    while i < len(anagrams):
        if is_anagram(word_primes_product, anagrams[i]) \
           and word.lower() != anagrams[i].lower():
            i += 1
        else:
            anagrams.pop(i)
    return anagrams
